...
class GreaterThan(_lhs: Expression[TypeInt], _rhs: Expression[TypeInt]) extends BinExpression[TypeBool, TypeInt, TypeInt](_lhs, _rhs, typeOf[TypeInt]) {
...
object GreaterThan {
	def apply(lhs: Expression[TypeInt], rhs: Expression[TypeInt]): GreaterThan = new GreaterThan(lhs, rhs)
}